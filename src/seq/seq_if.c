...
static int pv_cmp(const void *pl, const void *pr)
{
	int l = *(int *)pl;
	int r = *(int *)pr;
...
/*
 * lock a set of variables against concurrent access (including CA callbacks)
 */
epicsShareFunc void seq_pvLock(SS_ID ssId, int npvs, ...)
{
	...
	va_end(ap);
	qsort(pvs, npvs, sizeof(int), pv_cmp);
	for (i = 0; i < npvs; i++)
	{
		CHAN *pDB = ssId->sprog->pChan + pvs[i];
#ifdef DEBUG
		printf("%s: pvLock %d\n", ssId->pSSName, pvs[i]);
#endif
		epicsMutexLock(pDB->varLock);
...
/*
 * atomically copy content of assigned variable to pVal
 */
epicsShareFunc void epicsShareAPI seq_pvCopy(SS_ID ssId, long pvId, void *pVal)
{
	CHAN	*pDB = ssId->sprog->pChan + pvId;

	epicsMutexLock(pDB->varLock);
	memcpy(pVal, pDB->pVar, pDB->size * pDB->count);
	epicsMutexUnlock(pDB->varLock);
}
