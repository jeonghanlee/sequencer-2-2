...
threadId seqAuxThreadId = (threadId) 0;
 ...
 */
epicsShareFunc long epicsShareAPI seq (pSeqProg, macroDef, stackSize)
struct seqProgram	*pSeqProg;	/* state prog info generated by snc */
char			*macroDef;	/* optional macro def'n string */
unsigned int		stackSize;	/* optional stack size (bytes) */
{
	threadId	tid;
	...
	{	/* Oops */
		...
		threadSleep( 1.0 );	/* let error messages get printed */
		return -1;
	}
	...
	/* Specify stack size */
	if (stackSize == 0)
		stackSize = threadGetStackSize(THREAD_STACK_SIZE);
	pValue = seqMacValGet(pSP->pMacros, "stack");
	if (pValue != NULL && strlen(pValue) > 0)
	{
		sscanf(pValue, "%ud", &stackSize);
	}
	smallStack = threadGetStackSize(threadStackSmall);
	...
	if (seqAuxThreadId == (threadId) 0)
	{
		unsigned int auxStack = threadGetStackSize(threadStackSmall);
		threadCreate("seqAux", THREAD_PRIORITY+1, auxStack,
				(THREADFUNC)seqAuxThread, &auxArgs);
		while (seqAuxThreadId == (threadId) 0)
		    /* wait for thread to init. message system */
		    threadSleep(0.1);

		if (seqAuxThreadId == (threadId) -1)
		{
		    threadSleep( 1.0 );	/* let error messages get printed */
		    return -1;
		}
	...
	tid = threadCreate(pThreadName, pSP->threadPriority, pSP->stackSize,
...
SPROG			*pSP;
{
	...
	pSP->caSemId = semMutexMustCreate();
	pSP->connCount = 0;
	pSP->assignCount = 0;
	pSP->logFd = NULL;
...
SPROG			*pSP;
{
	...
	for (nss = 0; nss < pSeqProg->numSS; nss++, pSS++, pSeqSS++)
	{
		...
		pSS->syncSemId = semBinaryMustCreate(semFull);

		/* Create binary semaphores for synchronous pvGet() and
		   pvPut() */
		pSS->getSemId = semBinaryMustCreate(semFull);
		pSS->putSemId = semBinaryMustCreate(semFull);

		/* Create binary semaphores for thread death */
		pSS->death1SemId = semBinaryMustCreate(semEmpty);
		pSS->death2SemId = semBinaryMustCreate(semEmpty);
		pSS->death3SemId = semBinaryMustCreate(semEmpty);
		pSS->death4SemId = semBinaryMustCreate(semEmpty);
...
epicsStatus seq_logv(SPROG *pSP, const char *fmt, va_list args)
{
	...
	/* Enter thread name */
	sprintf(pBfr, "%s ", threadGetNameSelf() );
	pBfr += strlen(pBfr);

	/* Get time of day */
	tsStampGetCurrent(&timeStamp);	/* time stamp format */

	/* Convert to text format: "yyyy/mm/dd hh:mm:ss" */
	tsStampToStrftime(pBfr, eBfr-pBfr, "%Y/%m/%d %H:%M:%S", &timeStamp);
	pBfr += strlen(pBfr);
